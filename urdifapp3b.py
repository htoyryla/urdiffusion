...
def diffusion_run(modelsel, im, o,  progress, prompt_img = None):
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(min_width = 80):
            ...
            '<div style="font-size: 14px; color:#666666;">by @htoyryla 2023</div>'
            logo = gr.HTML(html)
    ...
    def run(o, imo, ms, t, s, m, im_in, skip, weak, prompt_img, imgw):
        ...
        for im, imr, i in diffusion_run(ms, im_in, o, gr.Progress(), prompt_img):
            imo = imr
            yield im, imr, imo, o, str(i)     
            
    # buttons to trigger diffusion and post processing        
             
    text_button.click(queue=True, fn=run, inputs=[opt_, imo, modelsel, text_input, textw, mul, init_image, skip, weak, prompt_img, imgw], outputs=[image_output, image_output_raw, imo, opt_, process_status])
